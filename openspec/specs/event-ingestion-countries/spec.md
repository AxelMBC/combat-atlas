# event-ingestion-countries Specification

## Purpose

Derive the admin event form's country options from the single source of truth (`countryRegistry`) so every registered country — current and future — is selectable without maintaining a parallel list.
