# theme-surface-tokens Specification

## Purpose

Expose the active `SurfacePalette` through a single MUI theme on every route so that surfaces, spinners, modals, header toggles, and shared card primitives all derive their colors from theme tokens rather than `useThemeMode()` or hardcoded hex, keeping the app consistent and mode-reactive.
