...
import type { FeedbackModalProps, FeedbackVariant, VariantStyles } from './FeedbackModal.types';

const VARIANT_STYLES: Record<FeedbackVariant, VariantStyles> = {
  success: {
    borderGradient: 'linear-gradient(135deg, #26c954 0%, #0e7a26 50%, #26c954 100%)',
    iconColor: '#26c954',
  ...
  error: {
    borderGradient: 'linear-gradient(135deg, #ff4d4d 0%, #7a0606 50%, #ff4d4d 100%)',
    iconColor: '#ff4d4d',
...
const FeedbackModal = ({
  ...
  const cardInitial = shouldReduceMotion ? { opacity: 0 } : { opacity: 0, scale: 0.6, y: 20 };
  const cardAnimate = shouldReduceMotion ? { opacity: 1 } : { opacity: 1, scale: 1, y: 0 };
  const cardExit = shouldReduceMotion ? { opacity: 0 } : { opacity: 0, scale: 0.8 };
  const cardTransition = shouldReduceMotion
    ? { duration: 0.2 }
    : { duration: 0.45, ease: [0.34, 1.56, 0.64, 1] as const };

  const shakeAnimate =
    variant === 'error' && !shouldReduceMotion ? { x: [0, -10, 10, -8, 8, -4, 4, 0] } : { x: 0 };
  const shakeTransition =
    variant === 'error' && !shouldReduceMotion
      ? { duration: 0.5, delay: 0.4, ease: 'easeInOut' as const }
      : { duration: 0 };

  const iconAnimate = shouldReduceMotion ? { scale: 1 } : { scale: [1, 1.15, 1] };
