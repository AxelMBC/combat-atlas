...
describe('fetchCountry thunk', () => {
  ...
  it('returns cached data without calling the service when cache is fresh', async () => {
    const payload = makePayload();
    sessionStorage.setItem('country_ar', JSON.stringify({ data: payload, fetchedAt: NOW - 1000 }));
  ...
  it('rejects with the country-data error key when the service throws an Error', async () => {
    mockedGetCountryData.mockRejectedValueOnce(new Error('network down'));

    const result = await runThunk('ar');

    expect(result.type).toBe(fetchCountry.rejected.type);
    expect(result.payload).toBe('error.countryData');
  });

  it('rejects with the country-data error key when the service throws a non-Error', async () => {
