...
const WorldMap = () => {
  ...
  const layerIdToEntry = useMemo<Record<string, (typeof countryRegistry)[number]>>(() => {
  ...
  const handleMapLoad = () => {
    ...
    const layerIds = countryRegistry.map((c) => c.mapLayerId).filter((id) => map.getLayer(id));
    ...
    const tick = (now: number) => {
      ...
      const opacity =
        PULSE_MIN + (PULSE_MAX - PULSE_MIN) * (0.5 + 0.5 * Math.sin(phase * Math.PI * 2));
...
// maplibre holds a WebGL context tied to its canvas. A hot-patch would remount
// the <Map> and stack a new context on top of the old one before the browser
// frees it; after enough reloads Chrome recycles the GPU process and WebGL
// creation starts failing. Self-accept + invalidate forces a full page reload on
// edits to this module so the context is released cleanly. Dev-only: stripped in build.
if (import.meta.hot) {
